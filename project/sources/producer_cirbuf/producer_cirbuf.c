...
{    
    // Creating logger
    logger = (Logger){"producer_cirbuf", atoi(argv[1])};
    ...
    if((ptr = mmap(0, array_length, PROT_WRITE, MAP_SHARED, shm_fd, 0)) == MAP_FAILED)
    ...
    for(int i=0; i<array_length; ++i)
    {
        ...
        if(sem_wait(sem_not_full) == -1)
            perror_exit(&logger, "Waiting on buff_not_full semaphore");
        ...
        if(sem_post(sem_not_empt) == -1)
            perror_exit(&logger, "Posting on buff_not_empty semaphore");
    }
